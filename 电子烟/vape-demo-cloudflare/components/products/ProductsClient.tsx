...
export default function ProductsClient({ products }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return result.sort((a, b) => {
      const aPrice = a.pricing.wholesaleTiers[0]?.unitPrice ?? 0
      const bPrice = b.pricing.wholesaleTiers[0]?.unitPrice ?? 0
                    ...
                    <div className="p-6 flex flex-col flex-1 border-t border-[#1a1a1a]">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <div className="text-[9px] font-black uppercase tracking-[0.2em] text-[#666]">
                          {p.series} · {t(`products.category.${p.category}`)}
                        </div>
                      </div>
                      
                      <h3 className="text-lg font-black text-white mb-2 uppercase tracking-tight line-clamp-1 group-hover:text-white transition-colors">{p.name[lang]}</h3>
                      <p className="text-xs text-[#666] mb-8 line-clamp-2 font-medium leading-relaxed">{p.tagline[lang]}</p>

...
function buildSpecRows(specs: VapeProduct['specs']): { label: string; value: string }[] {
  const rows: { label: string; value: string }[] = []
  if (specs.batteryCapacity) rows.push({ label: 'Battery', value: `${specs.batteryCapacity}mAh` })
  if (specs.eJuiceCapacity) rows.push({ label: 'E-Liquid', value: `${specs.eJuiceCapacity}mL` })
  if (specs.cartridgeCapacity) rows.push({ label: 'Cartridge', value: `${specs.cartridgeCapacity}mL` })
  if (specs.puffs) rows.push({ label: 'Puffs', value: `${specs.puffs.toLocaleString()}` })
  if (specs.nicotineStrengths && specs.nicotineStrengths.length > 0) rows.push({ label: 'Nicotine', value: specs.nicotineStrengths.map(n => `${n}mg`).join(' / ') })
  if (specs.wattageRange) rows.push({ label: 'Wattage', value: specs.wattageRange })
  if (specs.resistance) rows.push({ label: 'Resistance', value: `${specs.resistance}Ω` })
  if (specs.chargingPort) rows.push({ label: 'Charging', value: `${specs.chargingPort}${specs.chargingTime ? ` / ${specs.chargingTime}` : ''}` })
  if (specs.dimensions) rows.push({ label: 'Dimensions', value: specs.dimensions })
  if (specs.weight) rows.push({ label: 'Weight', value: `${specs.weight}g` })
  if (specs.coilType) rows.push({ label: 'Coil', value: specs.coilType })
  if (specs.airflowControl !== undefined) rows.push({ label: 'Airflow', value: specs.airflowControl ? 'Adjustable' : 'Fixed' })
  if (specs.displayScreen !== undefined) rows.push({ label: 'Display', value: specs.displayScreen ? 'Yes' : 'No' })
  if (specs.isRefillable !== undefined) rows.push({ label: 'Refillable', value: specs.isRefillable ? 'Yes' : 'No' })
  return rows
}
            ...
            <div className="grid grid-cols-4 gap-3">
              {product.images.map((img, i) => (
                <button
                  ...
                  className={`aspect-square bg-[#050505] border flex items-center justify-center transition-colors ${activeImageIndex === i ? 'border-white' : 'border-[#1a1a1a] hover:border-[#333]'}`}
                >
                  <span className="text-[9px] font-bold tracking-wider uppercase" style={{ color: activeImageIndex === i ? seriesColor : '#333' }}>
                    {img.type}
                  </span>
                </button>
              ))}
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-[#888]">
                {product.series} SERIES · {product.modelNumber}
              </span>
            </div>
            ...
            <div className="p-6 bg-[#050505] border border-[#1a1a1a] mb-6">
              ...
              <div className="flex gap-6 mb-6 pt-4 border-t border-[#111]">
                <div>
                  ...
                  <div className="text-lg font-black text-white">{product.moq.toLocaleString()} <span className="text-xs text-[#555]">units</span></div>
                </div>
                <div>
        ...
        {product.viewer3dUrl && (
          ...
          >
            ...
            >
              ...
              <span className="text-[11px] font-black uppercase tracking-[0.25em] text-white">3D View</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">WHOLESALE PRICING</h2>
              </div>
              <div className="overflow-hidden border border-[#1a1a1a]">
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-[#1a1a1a]">
                      <th className="px-5 py-3 text-[9px] font-black text-[#555] uppercase tracking-[0.2em]">Quantity</th>
                      <th className="px-5 py-3 text-[9px] font-black text-[#555] uppercase tracking-[0.2em]">Unit Price</th>
                      <th className="px-5 py-3 text-[9px] font-black text-[#555] uppercase tracking-[0.2em]">Savings</th>
                    </tr>
                  </thead>
                  <tbody>
                    {product.pricing.wholesaleTiers.map((tier, i) => {
                        ...
                        <tr key={i} className={`border-b border-[#0d0d0d] last:border-0 ${i === 0 ? 'bg-[#0d0d0d]' : ''}`}>
                          ...
                          <td className="px-5 py-4">
                            {saving > 0 ? (
                              ...
                              <span className="text-[9px] text-[#333]">BASE</span>
                            )}
                          </td>
                        </tr>
                      )
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-[11px] font-black text-white uppercase tracking-[0.2em]">LEAD TIME & MOQ</h2>
              </div>
              <div className="space-y-4 mb-8">
                <div className="flex justify-between py-4 border-b border-[#111]">
                  ...
                  <span className="text-sm font-black text-white">{product.moq.toLocaleString()} units</span>
                </div>
                <div className="flex justify-between py-4 border-b border-[#111]">
                  <span className="text-[10px] text-[#555] font-bold uppercase tracking-wider">Standard Lead Time</span>
                  <span className="text-sm font-black text-white">{product.leadTime.standard} days</span>
                </div>
                <div className="flex justify-between py-4 border-b border-[#111]">
                  <span className="text-[10px] text-[#555] font-bold uppercase tracking-wider">Custom / OEM</span>
                  <span className="text-sm font-black text-white">{product.leadTime.custom} days</span>
                </div>
                <div className="flex justify-between py-4">
                  <span className="text-[10px] text-[#555] font-bold uppercase tracking-wider">Retail Price (MSRP)</span>
              ...
              {relatedProducts.slice(0, 4).map(r => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[9px] font-black uppercase tracking-[0.2em] mb-1.5 text-[#555]">{r.series} SERIES</div>
                    ...
                    <div className="text-[9px] font-bold text-[#444] tracking-widest uppercase">
                      FROM <span className="text-[#aaa] text-sm font-black ml-1">${rPrice.toFixed(2)}</span>
                    </div>
                  </Link>
                )
              })}

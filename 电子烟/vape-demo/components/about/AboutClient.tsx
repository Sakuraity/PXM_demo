        ...
        <section className="py-24 border-b border-[#1a1a1a]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <motion.div 
              ...
              className="aspect-video bg-[#050505] border border-[#1a1a1a] flex items-center justify-center relative overflow-hidden group"
            >
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05)_0%,transparent_100%)] group-hover:scale-110 transition-transform duration-1000" />
              <div className="text-center relative z-10">
                <div className="text-5xl mb-4 grayscale opacity-50">🏭</div>
                <span className="text-[#444] text-[10px] font-bold tracking-[0.2em] uppercase">Factory Facility — 1920×1080px</span>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="space-y-4">
                {[
                  { icon: '🏗️', text: '30,000 sqm facility in Shenzhen' },
                  { icon: '🤖', text: 'Automated assembly lines' },
                  { icon: '🧪', text: 'ISO-certified clean rooms' },
                  { icon: '✅', text: '100-point quality inspection' },
                ].map(({ icon, text }) => (
                  <div key={text} className="flex items-center gap-4 p-4 bg-[#050505] border border-[#1a1a1a] hover:border-[#333] transition-colors group">
                    <span className="text-xl grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all">{icon}</span>
                    <span className="text-xs font-bold text-[#888] uppercase tracking-wider group-hover:text-white transition-colors">{text}</span>
                  </div>
                ))}
              </div>
            </motion.div>
